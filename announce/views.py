...
class AnnounceDetailView(LoginRequiredMixin, DetailView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        comment_save = Comment.objects.create(
            announce=self.object, comment=comment, author=self.request.user
        )
        comment_save.save()
...
class AnnounceUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if images:
                for image in images:
                    a_image = AnnounceImage.objects.create(
                        announce=form_id, images=image
                    )
                    a_image.save()
            else:
                form_save.save()
...
class AnnounceNotReadView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["not_read"] = self.request.user.announce_set.filter(
            ~Q(author=self.request.user) & Q(reads__id=self.request.user.id)
        ).count()
